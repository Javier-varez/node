...
int LPS25H_init(struct sensor_struct *sensor, void *handler, uint16_t sampling_period_s) {
	uint8_t rc = i2c_sensor_init(sensor, "LPS25H", LPS25H_ID, &LPS25H_func_table, (I2C_HandleTypeDef*)handler, LPS25H_DEV_ADDR << 1, &LPS25H_init_array,
								 PRESS_OUT_XL | AUTO_INCREMENT_ADDR, sizeof(uint8_t), sizeof(LPS25H_Out_Data),
								 WHO_AM_I, LPS25H_ID_VALUE, sampling_period_s ? sampling_period_s : DEFAULT_PERIOD_S);
	if (rc == 0) {
