...
Sensor_Probe_Intf LSM9DS1_AG_intf = {
...
int LSM9DS1_AG_init(struct sensor_struct *sensor, void *handler, uint16_t sampling_period_s) {
	uint8_t rc = i2c_sensor_init(sensor, "LSM9DS1_AG",LSM9DS1_AG_ID, &LSM9DS1_AG_func_table, (I2C_HandleTypeDef*)handler, LSM9DS1_AG_DEV_ADDR << 1, &LSM9DS1_AG_init_array,
								 OUT_X_L_G, sizeof(uint8_t), sizeof(int16_t), WHO_AM_I, LSM9DS1_AG_ID_VALUE, sampling_period_s ? sampling_period_s : DEFAULT_PERIOD_S);
