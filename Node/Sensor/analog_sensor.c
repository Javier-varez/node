...
int analog_sensor_init(Sensor *sensor, char *name, uint8_t sensorID,
		Sensor_Func_Table *func_tbl, uint16_t sampling_period_s, ADC_HandleTypeDef *hadc,
		uint8_t identifier) {
	...
	if (sensor != NULL) {
		...
		sensor->sampling_period_s = (sampling_period_s >= 0x7FFF) ? 0xFFFFFFFF : sampling_period_s;
